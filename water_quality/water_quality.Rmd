...
m <- leaflet() %>% 
  addTiles() %>% 
  addMarkers(lng = gwq_sites$Longitude, lat = gwq_sites$Latitude, popup = gwq_sites$WellName)
m
...
```{r}
selectInput("indicator", label = "Indicator", choices = c("E.coli", "Nitrate nitrogen"), selected)
...
renderLeaflet(
  ...
  addTiles() %>% 
  addMarkers(lng = g$Longitude, lat = g$Latitude, popup = g$WellName)
)
```
