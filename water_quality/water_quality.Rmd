...
mean_by_well <- gwq_sites %>% 
  group_by(Region, WellName, Latitude, Longitude, Indicator) %>% 
...
m <- leaflet() %>% 
  addTiles() %>% 
  addCircles(lng = gwq_sites$Longitude, lat = gwq_sites$Latitude, popup = gwq_sites$WellName)
m
...
-------------------------------------
```{r}
selectInput("indicator", label = "Indicator", choices = c("E.coli cfu/100ml", "Nitrate nitrogen g/m3"))
```
...
renderLeaflet(
  if(input$indicator == "E.coli cfu/100ml") {
    ...
    addCircleMarkers(lng = mean_by_well$Longitude, lat = mean_by_well$Latitude, 
                   popup = if_else(!is.na(mean_by_well$m), glue("{mean_by_well$WellName}: {round(mean_by_well$m, 2)} cfu/100ml"), glue("{mean_by_well$WellName}: Not Assessed")) ,
             color = if_else(!is.na(mean_by_well$m), "red", "grey"))
  } else {
    mean_by_well %>% 
    filter(Indicator == input$indicator) %>% 
    leaflet() %>% 
    ...
    addTiles() %>% 
    addCircleMarkers(lng = mean_by_well$Longitude, lat = mean_by_well$Latitude, 
                   popup = case_when(is.na(mean_by_well$m) ~ glue("{mean_by_well$WellName}: Not Assessed"), 
                                     mean_by_well$m > 11.3 ~ glue("{mean_by_well$WellName}: High Risk"),
                                     TRUE ~ glue("{mean_by_well$WellName}: Medium Risk")) ,
             color = case_when(is.na(mean_by_well$m) ~ "grey",
                               mean_by_well$m > 11.3 ~ "red",
                               TRUE ~ "green"))
  }
)
```
